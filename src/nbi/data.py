...
class BaseContainer(Dataset):
    def __init__(self, x, y, f_val=0.2, f_test=0, split='all', process=None):
        ...
        self.val = Data(x[int(N * p_train):int(N * p_val)], y[int(N * p_train):int(N * p_val)])
...
class ContainerDemo(BaseContainer):
    ...
    def noisify(self, total_flux):
        ...
        rand = self.rand if self.rand is not None else np.random.normal(0, 1, size=flux_err.shape[0])
