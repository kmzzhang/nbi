...
class MADEMOG(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, cond_label_size=None, n_components=5,
                 n_hidden=1, activation='relu', input_order='sequential',
                 input_degrees=None, clamp_0=-2, clamp_1=3, shift_only=False,
                 linear_scale=False):
        """
        Args:
    ...
    def forward(self, inputs, cond_inputs=None, mode='direct'):
        ...
        if mode == 'direct':
            ...
            for i in self.input_degrees:
                ...
                z = D.Categorical(logits=logr[..., i]).sample().unsqueeze(-1)  # out (N, B, 1)
...
class MADE(nn.Module):
    ...
    def __init__(self,
        ...
        output_mask = get_mask(
            num_hidden, output_dims, num_inputs, mask_type='output')
        self.clamp_0 = clamp_0 if not linear_scale else np.exp(clamp_0)
        self.clamp_1 = clamp_1 if not linear_scale else np.exp(clamp_1)
        ...
        self.trunk = nn.Sequential(act_func(),
                                   ...
                                   nn.MaskedLinear(num_hidden, output_dims,
                                                   output_mask))
        ...
        if mode == 'direct':
            ...
            for i_col in range(inputs.shape[-1]):
                if self.shift_only:
                    ...
                    m, a = self.trunk(h).chunk(2, -1)
                    a = torch.clamp(a, self.clamp_0, self.clamp_1)
                    x.transpose(0, -1)[i_col] = inputs.transpose(0, -1)[i_col] * torch.exp(a.transpose(0, -1)[i_col]) + \
...
class BatchNormFlow(nn.Module):
    ...
    def _add_stats(self, mean, var, n_batch):
        ...
        self.running_mean.add_(
            mean.data * n_batch / (self.count + n_batch)
        )
        self.running_var.add_(
            var.data * n_batch / (self.count + n_batch)
        )
        self.count += n_batch
...
class Shuffle(nn.Module):
    ...
    def forward(self, inputs, cond_inputs=None, mode='direct'):
        ...
        if mode == 'direct':
            ...
            return inputs, torch.zeros(
                *inputs.shape[:-1], 1, device=inputs.device)
        else:
...
class FlowSequential(nn.Sequential):
    ...
    def forward(self, inputs, cond_inputs=None, mode='direct', logdets=None, C=5):
        ...
        if logdets is None:
            if len(inputs.shape) == 2:
                ...
                logdets = torch.zeros(cond_inputs.shape[0], inputs.shape[1], 1, device=inputs.device)
    ...
    def sample(self, num_samples=None, noise=None, cond_inputs=None):
        ...
        if noise is None:
            torch.manual_seed(0)
            noise = torch.Tensor(1, num_samples, self.num_inputs).normal_().repeat(N, 1, 1)
        ...
        samples = self.forward(noise, cond_inputs, mode='inverse')[0]  # N, num_samples, dim
        return samples
    ...
    def entropy(self, num_samples=None, noise=None, cond_inputs=None):
        ...
        if cond_inputs is not None:
            cond_inputs = cond_inputs.to(device)
            cond_inputs = cond_inputs.unsqueeze(1)
        samples, log_jacob = self.forward(noise, cond_inputs, mode='inverse') # N, num_samples, dim
...
class FlowSequentialMOG(nn.Sequential):
    ...
    def sample(self, num_samples=None, noise=None, cond_inputs=None):
        ...
        if noise is None:
            noise = torch.Tensor(num_samples, 1, self.C, self.num_inputs).normal_().repeat(1, B, 1, 1)
        noise = noise.to(device)
